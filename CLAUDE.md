...
2. Call solver method (`mrt`/`ros3prw`/`ros34prw`/`rk3`/`rk4`/`ab5`/`ab4`/`rkdp`) → returns `Float64Array[]` (argument values + solution columns)
...
- `correctness.test.ts` — Validates solver accuracy against reference solutions (threshold: MAX_MAD = 0.1)
  - Tests all methods (implicit + explicit) against 6 problems with known exact solutions (8 methods total)
...
- Method definitions in `test-defs.ts`: `methods` map (all 8 solvers), `implicitMethods` map (stiff-capable only)
