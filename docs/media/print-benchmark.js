...
import { mrt, ros34prw } from '../../index';
/** Solve the given problem and output to console the solution */
export function printSolution(task, method, separator) {
    try {
        // Solve the problem
        const solution = method(task);
        // Output results
        console.log(task.name);
        let line = [task.arg.name].concat(task.solutionColNames).join(separator);
        console.log(line);
...
```
 */
export function printRobertson() {
...
print(f"Solution saved to '{filename}'")
 */
export function printPollution() {
    printSolution(pollution, ros34prw, SEPARATOR);
}
//# sourceMappingURL=print-benchmark.js.map