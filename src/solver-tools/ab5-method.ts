/* The Adams-Bashforth-Moulton predictor-corrector method of order 5 (AB5).

   Uses the 5-step Adams-Bashforth formula as predictor and the 4-step
...
// Adams-Bashforth 5-step coefficients (pre-divided by 720)
// y_{n+1} = y_n + h * (AB0*f_n + AB1*f_{n-1} + AB2*f_{n-2} + AB3*f_{n-3} + AB4*f_{n-4})
const AB0 = 1901.0 / 720.0;
const AB1 = -2774.0 / 720.0;
const AB2 = 2616.0 / 720.0;
const AB3 = -1274.0 / 720.0;
const AB4 = 251.0 / 720.0;

// Adams-Moulton 4-step coefficients (pre-divided by 720)
// y_{n+1} = y_n + h * (AM_NEW*f_{n+1} + AM0*f_n + AM1*f_{n-1} + AM2*f_{n-2} + AM3*f_{n-3})
const AM_NEW = 251.0 / 720.0;
const AM0 = 646.0 / 720.0;
const AM1 = -264.0 / 720.0;
const AM2 = 106.0 / 720.0;
const AM3 = -19.0 / 720.0;

// Milne error estimation coefficient for the AB5-AM4 pair: 2/27 = 38/513
const MILNE = 2.0 / 27.0;

// Number of uniformly-spaced history points required for AB5
const HIST_LEN = 5;
...
/** Solve initial value problem using the Adams-Bashforth-Moulton predictor-corrector method of order 5
 ...
 * @example
 * // Example. Solve the following initial value problem using the AB5 method:
 * //   dx/dt = x + y - t
 * //   dy/dt = x * y + t
 * //   x(0) = 1
 * //   y(0) = -1
 * // on [0, 2] with the step 0.01.
 * import { ODEs, ab5 } from 'diff-grok';
 *
 * const task: ODEs = {
 ...
 *
 * try {
 *   // Solve the problem using the AB5 method
 *   const solution = ab5(task);
 *
 ...
 * }
*/
export function ab5(odes: ODEs, callback?: Callback): Float64Array[] {
  const f = odes.func;
  ...
  // AB5-AM4 history: fBuf[0] = f(t_n, y_n), fBuf[4] = f(t_{n-4}, y_{n-4})
  ...
  while (flag) {
    ...
    // if step size changed, history is invalid (AB5 needs uniform spacing)
    ...
    while (true) {
      if (histCount < HIST_LEN) {
        ...
        if (errmax > 1) {
          ...
          tNew = t + h;
          if (tNew == t)
            throw new Error(ERROR_MSG.AB5_FAILS);
        ...
        // AB5-AM4 PHASE: Predictor-Corrector (PECE)

        // Predict (Adams-Bashforth 5-step)
        for (let i = 0; i < dim; ++i) {
          yTemp[i] = y[i] + h * (AB0 * fBuf[0][i] + AB1 * fBuf[1][i] +
            AB2 * fBuf[2][i] + AB3 * fBuf[3][i] + AB4 * fBuf[4][i]);
        }

        // Evaluate f at predicted point
        f(t + h, yTemp, fPred);

        // Correct (Adams-Moulton 4-step)
        for (let i = 0; i < dim; ++i) {
          yCorr[i] = y[i] + h * (AM_NEW * fPred[i] + AM0 * fBuf[0][i] +
            AM1 * fBuf[1][i] + AM2 * fBuf[2][i] + AM3 * fBuf[3][i]);
        }
